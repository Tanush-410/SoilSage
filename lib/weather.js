...
export async function geocodeCity(city) {
  try {
    const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(city)}&format=json&limit=1`, {
      headers: { 'User-Agent': 'SoilSage-Hackathon/1.0' }
    })
    const data = await res.json()
    if (data?.[0]) return { latitude: parseFloat(data[0].lat), longitude: parseFloat(data[0].lon), name: data[0].display_name }
    
    const res2 = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(city)}&count=1`)
    const data2 = await res2.json()
    if (data2.results?.[0]) return { latitude: data2.results[0].latitude, longitude: data2.results[0].longitude, name: data2.results[0].name }

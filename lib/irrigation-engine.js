/**
 * ============================================================
 * AquaAI Custom Irrigation Engine
...
export function computeIrrigationRecommendation({ crop, soilData, weather, fieldArea, location }) {
  ...
  const efficiencyScore = Math.min(98, Math.round(72 + (RAW - depletionMM) / TAW * 20 - phPenalty - nPenalty + Math.random() * 5))
...
export function generateWeeklyPlan({ crop, soilData, weather, fieldArea }) {
  ...
  return weather.daily?.slice(0, 7).map((d, i) => {
    ...
    const netIrr = Math.max(0, etc - rain * 0.75)
    const liters = netIrr * 10000 * fieldArea
    ...
    return {
      ...
      etc: etc.toFixed(1),
      rain: rain.toFixed(0),
      netIrrigation: netIrr.toFixed(1),
      liters: Math.round(liters),
      irrigate: liters > 100 && rain < 8,
